...
class GetWeatherTool extends StructuredTool {
  ...
  schema = z.object({
    city: z.string().describe("The city and country ONLY, e.g. 'New York, US'. Do not include the state or other descriptors.")
  });
...
class CalculatorTool extends StructuredTool {
  name = "Calculator";
  description = "A simple calculator that can add two numbers. Be precise with input types.";

  schema = z.object({
    a: z.number().describe("The first number to add. String values are not accepted."),
    b: z.number().describe("The second number to add. String values are not accepted.")
  });
